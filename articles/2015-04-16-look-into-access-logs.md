...
P.S. Еще немного шпаргалок:

```sh
ssh host cat /var/log/nginx/access.log | awk -F' ' '{print $1 }' | uniq -c | sort -n -r
```

- Читает файл `/var/log/nginx/access.log` на удаленном хосте через ssh
- Разбивает каждую строку на подстроки используя пробел `awk -F' '` и возвращает первую подстроку `'{print $1 }'` в stdout
- `uniq -c` оставляет только уникальные значения добавляя счетчик повторений
- `sort -n -r` сортирует по числам от большего к меньшему
